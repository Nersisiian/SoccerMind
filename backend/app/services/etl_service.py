...
async def fetch_live_odds(db: AsyncSession):
    if not settings.ODDS_API_KEY:
        logger.warning("ODDS_API_KEY не задан – пропускаем сбор odds.")
        return
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.error(f"Ошибка получения odds: {e.response.status_code} {e.response.text}")
            return
    ...
    for game in data:
        ...
        await db.commit()
        logger.info(f"Сохранены odds для матча {home_team.name} vs {away_team.name}")

    logger.info("Сбор live‑odds завершён.")
